...
#define FORACE_CRASH_TIMEOUT_SEC 30

static void force_crash_timer_handler(unsigned long data)
{
	char *killer = NULL;
	pr_err("*************FORCE CRASH***************");
	// Write value to a NULL pointer to cause crash
	*killer = 1;
}
		        ...
		        unsigned int code, int value)
{
	if ((type == EV_KEY) && (code == KEY_POWER)) {
		if (value) {
			mod_timer(&force_crash_timer, jiffies + msecs_to_jiffies(FORACE_CRASH_TIMEOUT_SEC * 1000));
		} else {
